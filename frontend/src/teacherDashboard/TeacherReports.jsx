...
export default function TeacherReports() {
  ...
  const engagement = ["Recorded lectures saw a 12% rise in watch-through this week.", "Students open short-form revision videos 1.7x more than long notes.", "Attendance dips most sharply in the last session block of the day."];
  ...
  return (
    <div className="space-y-6">
      <section className={`rounded-[32px] border p-6 md:p-8 ${cardClass}`}>
        <p className={`text-xs font-semibold uppercase tracking-[0.22em] ${accentClass}`}>Reports</p>
        ...
        <p className={`mt-3 max-w-3xl text-sm leading-7 md:text-base ${mutedClass}`}>These reports highlight the places where progress is compounding and the topics that may need more explanation, more practice, or better scheduling.</p>
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.05fr_0.95fr]">
        <div className={`rounded-[28px] border p-5 ${cardClass}`}>
          <p className={`text-xs font-semibold uppercase tracking-[0.22em] ${accentClass}`}>Course Performance</p>
            ...
            {coursePerformance.map((item) => (
              <div key={item.label}>
                ...
                <div className={`mt-2 h-3 rounded-full ${isDarkTheme ? "bg-white/8" : "bg-slate-100"}`}>
                  <div className={`h-3 rounded-full ${isDarkTheme ? "bg-gradient-to-r from-indigo-400 via-violet-400 to-fuchsia-400" : "bg-gradient-to-r from-blue-500 via-indigo-500 to-fuchsia-500"}`} style={{ width: `${item.value}%` }} />
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="grid gap-4">
          <div className={`rounded-[28px] border p-5 ${cardClass}`}>
            ...
            <div className="mt-4 space-y-3">
              {weakTopics.map((topic) => (
                <div key={topic} className={`rounded-2xl border p-4 ${isDarkTheme ? "border-white/10 bg-white/5" : "border-slate-200/80 bg-slate-50/80"}`}>
          ...
          <div className={`rounded-[28px] border p-5 ${cardClass}`}>
            ...
            <div className="mt-4 space-y-3">
              {engagement.map((item) => (
                <div key={item} className={`rounded-2xl border p-4 ${isDarkTheme ? "border-white/10 bg-white/5" : "border-slate-200/80 bg-slate-50/80"}`}>
