  ...
  return (
    <div className="space-y-6">
      <section className="grid gap-4 xl:grid-cols-[0.85fr_1.15fr]">
        <div className={`rounded-[32px] border p-6 md:p-8 ${cardClass}`}>
          ...
          <div className="mt-5 flex items-center gap-4">
            <div className={`flex h-20 w-20 items-center justify-center rounded-full text-2xl font-bold ${isDarkTheme ? "bg-fuchsia-500/12 text-fuchsia-200" : "bg-indigo-100 text-indigo-700"}`}>MS</div>
            <div>
              ...
              <p className={`mt-2 text-xs font-semibold uppercase tracking-[0.18em] ${accentClass}`}>Teacher</p>
            </div>
          </div>

          <div className={`mt-6 rounded-2xl border p-4 ${isDarkTheme ? "border-white/10 bg-white/5" : "border-slate-200/80 bg-slate-50/80"}`}>
            <p className="text-sm font-semibold">Profile Snapshot</p>
            <p className={`mt-2 text-sm leading-7 ${mutedClass}`}>Strong live-class engagement, fast doubt resolution turnaround, and consistently above-target attendance across core science batches.</p>
          </div>
        </div>
      ...
      <section className={`rounded-[28px] border p-5 ${cardClass}`}>
        ...
        <div className="mt-5">
          <button type="button" className={`rounded-2xl px-5 py-3 text-sm font-semibold text-white transition hover:scale-[1.01] ${isDarkTheme ? "bg-gradient-to-r from-indigo-500/85 via-violet-500/85 to-fuchsia-500/85" : "bg-gradient-to-r from-blue-500 via-indigo-500 to-fuchsia-500"}`}>
            Save Changes
          </button>
        </div>
      </section>
