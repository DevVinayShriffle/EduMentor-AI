...
export default function TeacherHome() {
  ...
  const actions = ["Create Live Class", "Upload Material", "Create Test", "Review Doubts"];
  const activity = ["Uploaded projectile motion worksheet for Class 11 Foundation", "Published recording for Differential Calculus masterclass", "Reviewed 24 quiz submissions from the weekend batch", "Replied to 8 AI-escalated doubts in the chemistry group"];
  ...
  return (
    <div className="space-y-6">
      <section className={`rounded-[32px] border p-6 md:p-8 ${cardClass}`}>
        <p className={`text-xs font-semibold uppercase tracking-[0.24em] ${accentClass}`}>Teacher Dashboard</p>
        <h1 className="mt-4 text-3xl font-bold tracking-tight md:text-4xl">Keep classes, content, and student support moving from one control surface.</h1>
        <p className={`mt-3 max-w-3xl text-sm leading-7 md:text-base ${mutedClass}`}>Today&apos;s workspace surfaces live teaching priorities, pending student help, and the fastest actions you are likely to need before your next session starts.</p>
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
        <div className={`rounded-[28px] border p-5 ${cardClass}`}>
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className={`text-xs font-semibold uppercase tracking-[0.22em] ${accentClass}`}>Today&apos;s Schedule</p>
              <h2 className="mt-2 text-2xl font-semibold tracking-tight">Classes and checkpoints</h2>
            </div>
            <span className={`rounded-full px-3 py-1 text-xs font-semibold ${isDarkTheme ? "bg-white/8 text-slate-200" : "bg-blue-50 text-blue-700"}`}>3 live items</span>
          </div>
        ...
        <div className={`rounded-[28px] border p-5 ${cardClass}`}>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-1">
            {actions.map((action) => (
              <button key={action} type="button" className={`rounded-2xl px-4 py-3 text-left text-sm font-semibold transition hover:scale-[1.01] ${isDarkTheme ? "bg-gradient-to-r from-indigo-500/70 via-violet-500/70 to-fuchsia-500/70 text-white shadow-[0_16px_34px_rgba(129,140,248,0.22)]" : "bg-gradient-to-r from-blue-500 via-indigo-500 to-fuchsia-500 text-white shadow-[0_16px_34px_rgba(79,70,229,0.22)]"}`}>
                {action}
              </button>
            ))}
          </div>
          ...
          <div className={`mt-5 rounded-2xl border p-4 ${isDarkTheme ? "border-white/10 bg-white/5" : "border-slate-200/80 bg-slate-50/80"}`}>
            ...
            <p className={`mt-2 text-sm leading-6 ${mutedClass}`}>Reply to open doubts from the NEET chemistry batch, then publish the attendance recap before the 12:00 PM session starts.</p>
          </div>
        </div>
      </section>

      <section className={`rounded-[28px] border p-5 ${cardClass}`}>
        <p className={`text-xs font-semibold uppercase tracking-[0.22em] ${accentClass}`}>Recent Activity</p>
        <div className="mt-5 space-y-3">
          {activity.map((item, index) => (
            <div key={item} className={`flex items-start gap-3 rounded-2xl border p-4 ${isDarkTheme ? "border-white/10 bg-white/5" : "border-slate-200/80 bg-slate-50/80"}`}>
              <div className={`mt-1 flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold ${isDarkTheme ? "bg-fuchsia-500/15 text-fuchsia-200" : "bg-blue-100 text-blue-700"}`}>{index + 1}</div>
