  ...
  return (
    <div className="space-y-6">
      <section className={`rounded-[32px] border p-6 md:p-8 ${cardClass}`}>
        <p className={`text-xs font-semibold uppercase tracking-[0.22em] ${accentClass}`}>Content Management</p>
        <div className="mt-4 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          <div>
            ...
            <p className={`mt-3 max-w-3xl text-sm leading-7 md:text-base ${mutedClass}`}>Use this space to keep published content current, spot drafts waiting on review, and move lesson assets into the right course flow quickly.</p>
          </div>
        ...
        {sections.map((section) => (
          <article key={section.title} className={`rounded-[28px] border p-5 ${cardClass}`}>
            <div className="flex items-center justify-between gap-3">
              ...
              <span className={`rounded-full px-3 py-1 text-xs font-semibold ${isDarkTheme ? "bg-white/8 text-slate-200" : "bg-blue-50 text-blue-700"}`}>{section.rows.length} items</span>
            </div>

            <div className="mt-5 space-y-3">
              {section.rows.map((row) => (
                <div key={row.name} className={`rounded-2xl border p-4 ${isDarkTheme ? "border-white/10 bg-white/5" : "border-slate-200/80 bg-slate-50/80"}`}>
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <span className={`rounded-full px-3 py-1 text-xs font-semibold ${isDarkTheme ? "bg-fuchsia-500/12 text-fuchsia-200" : "bg-indigo-100 text-indigo-700"}`}>{row.status}</span>
                  </div>
                </div>
              ))}
            </div>
