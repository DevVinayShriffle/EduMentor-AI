    ...
    <div className="space-y-6">
      <section className={`rounded-[32px] border p-6 md:p-8 ${cardClass}`}>
        ...
        <p className={`mt-3 max-w-3xl text-sm leading-7 md:text-base ${mutedClass}`}>
          Use AI-assisted explanation drafting to clear repetitive confusion quickly, then send a more
          human reply where the student needs reassurance or extra steps.
        </p>
      </section>
            ...
            {doubts.map((item, index) => (
              <button
                ...
                className={`w-full rounded-2xl border p-4 text-left transition ${
                  isDarkTheme
                    ? index === 0
                      ? "border-fuchsia-400/30 bg-white/8"
              ...
              >
                ...
                <p className={`mt-1 text-xs font-semibold uppercase tracking-[0.18em] ${accentClass}`}>
                  {item.topic}
                </p>
        ...
        <div className={`rounded-[28px] border p-5 ${cardClass}`}>
          ...
          >
            ...
            <p className={`mt-3 text-sm leading-7 ${mutedClass}`}>
              Anti-Markovnikov addition depends on a radical pathway, and peroxide helps create that
              pathway. Without peroxide, the reaction usually follows the normal ionic addition route,
              so the more stable carbocation direction dominates instead.
            </p>
            <p className={`mt-3 text-sm leading-7 ${mutedClass}`}>
              A useful teaching follow-up is to compare the reaction mechanism step by step and ask the
              student which intermediate is formed in each case.
            </p>
          </div>
