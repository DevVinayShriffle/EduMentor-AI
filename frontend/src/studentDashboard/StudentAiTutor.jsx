  ...
  return (
    <div className="space-y-6">
      <section className={`rounded-[32px] border p-6 md:p-8 ${cardClass}`}>
        ...
        <h1 className="mt-4 text-3xl font-bold tracking-tight md:text-4xl">Ask doubts, review AI explanations, and keep a history of what confused you.</h1>
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
        <div className={`rounded-[28px] border p-5 ${cardClass}`}>
          ...
          <div className="mt-4 space-y-3">
            {doubts.map((item, index) => (
              <button key={item.topic + item.time} type="button" className={`w-full rounded-2xl border p-4 text-left transition ${isDarkTheme ? (index === 0 ? "border-cyan-400/30 bg-white/8" : "border-white/10 bg-white/5 hover:bg-white/8") : index === 0 ? "border-sky-200 bg-sky-50/80" : "border-slate-200/80 bg-slate-50/80 hover:bg-white"}`}>
        ...
        <div className={`rounded-[28px] border p-5 ${cardClass}`}>
          ...
          <div className={`mt-5 rounded-2xl border p-4 ${isDarkTheme ? "border-white/10 bg-white/5" : "border-slate-200/80 bg-slate-50/80"}`}>
            ...
            <p className={`mt-3 text-sm leading-7 ${mutedClass}`}>Start by checking the inner expression and asking whether its derivative is already present nearby. In timed tests, substitution is often the fastest when the expression repeats in multiple terms or appears inside a root, log, or trigonometric form.</p>
          </div>
          <div className="mt-5 flex flex-wrap gap-3">
            <button type="button" className={`rounded-2xl px-4 py-3 text-sm font-semibold text-white ${isDarkTheme ? "bg-gradient-to-r from-cyan-500 via-blue-500 to-indigo-500" : "bg-gradient-to-r from-sky-500 via-blue-500 to-indigo-500"}`}>
              Ask New Doubt
            </button>
            <button type="button" className={`rounded-2xl border px-4 py-3 text-sm font-semibold ${isDarkTheme ? "border-white/10 bg-white/5 text-slate-100" : "border-slate-200 bg-white text-slate-900"}`}>
              View History
            </button>
          </div>
        </div>
