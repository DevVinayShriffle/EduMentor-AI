  ...
  return (
    <div className="space-y-6">
      <section className={`rounded-[32px] border p-6 md:p-8 ${cardClass}`}>
        ...
        <h1 className="mt-4 text-3xl font-bold tracking-tight md:text-4xl">Keep upcoming sessions, attendance, and replay access in one view.</h1>
      </section>
            ...
            <div key={item.title} className={`rounded-2xl border p-4 ${isDarkTheme ? "border-white/10 bg-white/5" : "border-slate-200/80 bg-slate-50/80"}`}>
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex gap-3">
                  ...
                  <button type="button" className={`rounded-2xl px-4 py-2 text-sm font-semibold text-white ${isDarkTheme ? "bg-gradient-to-r from-cyan-500 via-blue-500 to-indigo-500" : "bg-gradient-to-r from-sky-500 via-blue-500 to-indigo-500"}`}>
                    Open
                  </button>
                </div>
              </div>
