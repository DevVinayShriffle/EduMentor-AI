import { BarChart3, BookOpen, Bot, CalendarDays, CreditCard, Home, LogOut, User, X } from "lucide-react";
...
export default function StudentSidebar({ isOpen, isDarkTheme, onBackToSite, onClose, onLogout }) {
  ...
  return (
    <>
      ...
      <aside className={`fixed left-0 top-0 z-50 flex h-dvh w-[18.5rem] flex-col border-r transition-transform duration-300 lg:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"} ${isDarkTheme ? "border-white/10 bg-[radial-gradient(circle_at_top_left,_rgba(6,182,212,0.24),_transparent_30%),radial-gradient(circle_at_top_right,_rgba(59,130,246,0.18),_transparent_24%),linear-gradient(180deg,_rgba(7,10,24,0.98)_0%,_rgba(10,18,33,0.98)_48%,_rgba(17,24,39,0.98)_100%)] text-slate-100 shadow-[0_24px_80px_rgba(2,6,23,0.45)]" : "border-slate-200/80 bg-[radial-gradient(circle_at_top_left,rgba(14,165,233,0.16),transparent_28%),linear-gradient(180deg,rgba(248,250,252,0.98)_0%,rgba(240,249,255,0.98)_100%)] text-slate-900 shadow-[0_24px_80px_rgba(15,23,42,0.12)]"}`}>
        <div className="px-5 py-5">
          <div className="flex items-center gap-3">
            <button type="button" onClick={onBackToSite} className="flex min-w-0 flex-1 items-center gap-3 rounded-2xl text-left transition hover:opacity-90" aria-label="Go to landing page" title="Back to landing page">
              <div className={`flex h-14 w-14 items-center justify-center rounded-full ${isDarkTheme ? "bg-white shadow-[0_0_22px_rgba(255,255,255,0.72),0_0_50px_rgba(56,189,248,0.35),0_14px_38px_rgba(15,23,42,0.45)]" : ""}`}>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-5">
          ...
          <div className={`mt-8 rounded-[28px] border px-4 py-4 ${isDarkTheme ? "border-white/10 bg-white/5" : "border-white/70 bg-white/80"}`}>
            <p className={`text-xs font-semibold uppercase tracking-[0.24em] ${isDarkTheme ? "text-cyan-200" : "text-sky-700"}`}>Quick Actions</p>
