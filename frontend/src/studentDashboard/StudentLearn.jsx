  ...
  return (
    <div className="space-y-6">
      <section className={`rounded-[32px] border p-6 md:p-8 ${cardClass}`}>
        ...
        <h1 className="mt-4 text-3xl font-bold tracking-tight md:text-4xl">Manage courses, recordings, shorts, and study materials from one place.</h1>
        <p className={`mt-3 max-w-3xl text-sm leading-7 md:text-base ${mutedClass}`}>This learning workspace is built for restarting quickly, continuing unfinished lessons, and keeping revision assets close to the core course flow.</p>
      </section>
              ...
              {section.rows.map((row) => (
                <div key={row.name} className={`rounded-2xl border p-4 ${isDarkTheme ? "border-white/10 bg-white/5" : "border-slate-200/80 bg-slate-50/80"}`}>
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <span className={`rounded-full px-3 py-1 text-xs font-semibold ${isDarkTheme ? "bg-cyan-500/12 text-cyan-200" : "bg-sky-100 text-sky-700"}`}>{row.status}</span>
                  </div>
                </div>
              ))}
            </div>
