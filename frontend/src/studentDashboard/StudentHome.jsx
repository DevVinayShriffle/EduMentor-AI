...
export default function StudentHome() {
  ...
  const activity = [
    "Finished 2 revision shorts from the electrostatics module",
    "Attempted 15 MCQs in the chemistry rapid drill",
    "Saved one AI doubt about integration substitutions",
    "Downloaded the latest biology notes pack",
  ];
  const upcoming = [
    ...
    <div className="space-y-6">
      <section className={`rounded-[32px] border p-6 md:p-8 ${cardClass}`}>
        ...
        <h1 className="mt-4 text-3xl font-bold tracking-tight md:text-4xl">
          Keep learning momentum visible, structured, and easy to restart.
        </h1>
        <p className={`mt-3 max-w-3xl text-sm leading-7 md:text-base ${mutedClass}`}>
          Your dashboard surfaces the next class, practice work, study streaks, and the fastest path
          back into revision when you return.
        </p>
      </section>
          ...
          <div className="mt-5 grid gap-3">
            {quickActions.map((action) => (
              <button
                key={action}
                type="button"
                className={`rounded-2xl px-4 py-3 text-left text-sm font-semibold text-white transition hover:scale-[1.01] ${
                  isDarkTheme
            ...
            >
              <div
                className={`mt-1 flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold ${
                  isDarkTheme ? "bg-cyan-500/15 text-cyan-200" : "bg-sky-100 text-sky-700"
                }`}
              >
                {index + 1}
              </div>
