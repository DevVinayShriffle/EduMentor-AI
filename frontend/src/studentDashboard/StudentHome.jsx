...
export default function StudentHome() {
  ...
  return (
    ...
    >
      <p className={`text-xs font-semibold uppercase tracking-[0.24em] ${isDarkTheme ? "text-cyan-300" : "text-sky-700"}`}>
        Student Route
      </p>
      <h1 className="mt-4 text-3xl font-bold tracking-tight">Student Home</h1>
      <p className={`mt-3 max-w-2xl text-sm leading-7 ${isDarkTheme ? "text-slate-300" : "text-slate-600"}`}>
        Phase 2 establishes the routed student workspace shell. This page now renders inside the
        dedicated student layout and receives theme state from the parent outlet context.
      </p>
    </div>
  );
}
