...
export default function StudentPayments() {
  ...
  const stats = [
    ...
    { label: "Renewal Date", value: "25 May 2026" },
    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-4 md:grid-cols-3">
        {stats.map((item) => (
          <article key={item.label} className={`rounded-[28px] border p-5 ${cardClass}`}>
            ...
            <h2 className="mt-4 text-2xl font-bold tracking-tight">{item.value}</h2>
          </article>
        ))}
      </section>
      ...
      <section className={`rounded-[28px] border p-5 ${cardClass}`}>
        ...
        <div className="mt-5 space-y-3">
          {history.map((item) => (
            <div key={item.item + item.date} className={`rounded-2xl border p-4 ${isDarkTheme ? "border-white/10 bg-white/5" : "border-slate-200/80 bg-slate-50/80"}`}>
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <h3 className="text-base font-semibold">{item.item}</h3>
                  <p className={`mt-1 text-sm ${mutedClass}`}>{item.date}</p>
                </div>
